...
**Addendum (added post-implementation, closing a self-identified gap):** the step budget alone is a blunt stop condition -- nothing previously detected a confused LLM re-issuing the exact same tool call turn after turn, which would burn several real LLM round trips (the dominant cost/latency driver) before `MAX_AGENT_STEPS` finally cut it off. Two complementary mechanisms added to `src/agent/graph.py`:
1. **Per-run memoization** of repeat calls to a pure-read tool (`query_segment`, `search_guidelines`) -- a repeat is served from an in-memory `tool_call_cache` instead of re-hitting SQLite/FAISS. Real for `search_guidelines` specifically, since it calls the embedding API (an actual external cost); `query_segment` is a local-only query, so its savings there are more a correctness/hygiene guarantee than a $ one.
2. **Early loop detection**: memoization alone only cheapens the *tool* side of a repeat -- the `tools -> agent` edge is unconditional, so the LLM would still get another full (paid) turn regardless. Instead, `call_model` checks the LLM's *proposed* tool_calls the instant they come back: if every call in that batch is an exact repeat of something already cached, that turn gained zero new information, so `loop_detected` is set immediately and `should_continue` routes straight to `fallback` -- skipping both the redundant dispatch and any further LLM turns, rather than paying for several more rounds before the step budget catches it. `create_campaign` is excluded from both mechanisms -- it writes, and its args are mutated in-flight by the compliance-citation/idempotency-key overrides, so short-circuiting it would risk bypassing that logic rather than saving real work (its repeat-call cost is already bounded by the idempotency unique-constraint fast path). A mixed batch (one new call + one repeat in the same turn) is not treated as a full loop -- it still dispatches normally, with only the repeat served from cache.
